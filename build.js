...
const { generateSitemapIndex, generateSubSitemap, generateLlmSitemap, generateImageSitemap, generateRobots } = require('./lib/sitemap-generator');
...
if (templates['city-landing']) {
  ...
  for (const city of cities) {
    const ctx = {
      ...
      regionName: city.region, cityZip: city.zip || '32920', cityPopulation: city.population,
      cityLat: city.lat || 28.3922, cityLng: city.lng || -80.6077,
...
if (templates['service-hub']) {
  ...
  for (const svc of services) {
    ...
    const html = render(templates['service-hub'], {
      ...
      serviceFeatures: svc.features || [],
      faqs: svc.faqs || [],
      faqSchema: buildFaqSchema((svc.faqs || []).slice(0, 5)),
      topCities,
...
const staticMeta = {
  ...
  'cookies': { title: 'Cookie Policy | BusBrother', desc: 'BusBrother Cookie Policy. Learn what cookies and tracking technologies we use on busbrother.com and how to control them.' },
...
// Image sitemap - Google Images discovery
const imageList = [];
for (const [key, img] of Object.entries(images)) {
  if (!img || !img.file) continue;
  const pageUrl = (img.services && img.services.length) ? `/services/${img.services[0]}/` : '/';
  imageList.push({ file: img.file, alt: img.alt || '', title: img.title || img.alt || '', page: pageUrl });
}
if (imageList.length) {
  generateImageSitemap(DIST, imageList);
  console.log('Image sitemap generated with', imageList.length, 'images');
}
